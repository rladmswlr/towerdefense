...
export const attackTower = (userId, payload) => {
    ...
    // 해당 위치의 타워가 존재하는지 체크
    if(tower.towerpos.x != payload.towerpos.x && 
        tower.towerpos.y != payload.towerpos.y ) {
            return {status: 'fail', message: 'Position is Not Matching'};
    }; 

    const towerdistance = Math.sqrt( Math.pow(tower.towerpos.x - monster.x, 2) + Math.pow(tower.towerpos.y - monster.y, 2),);
